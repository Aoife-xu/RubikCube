...
class Quaternion:
    ...
    @classmethod
    def from_v_theta(cls, v, theta):# cos 0.5theta, sin 0.5theta, 0, 0
        """
        ...
        """
        theta = np.asarray(theta) # Convert the input to an array
        v = np.asarray(v) # 1,00,
        s = np.sin(0.5 * theta) #0.7071067811865476
        c = np.cos(0.5 * theta) #0.7071067811865476

        v = v * s / np.sqrt(np.sum(v * v, -1)) # [0.70710678 0.         0.        ]
        x_shape = v.shape[:-1] + (4,) # ()+(4,)

        x = np.ones(x_shape).reshape(-1, 4)# [[1. 1. 1. 1.]]
        x[:, 0] = c.ravel() # 将数组c拉成一维数组, [0.70710678] 赋值给[0]
        x[:, 1:] = v.reshape(-1, 3) #[0.70710678]
        x = x.reshape(x_shape) # [0.70710678 0.70710678 0.         0.        ]
    ...
    def as_v_theta(self):
        ...
        # compute theta
        norm = np.sqrt((x ** 2).sum(0)) # sum=[1.]
        #[[0.5] =0.707^2
        # [0.5]
        # [0. ]
        # [0. ]]
        theta = 2 * np.arccos(x[0] / norm) # [1.57079633]=0.5pi

        # compute the unit vector
        v = np.array(x[1:], order='F', copy=True) #  切片，从某行到最后一行
        # [[0.70710678]
        #  [0.        ]
        #  [0.        ]]
...
def project_points(points, q, view, vertical=[0, 1, 0]):
    ...
    xdir = np.cross(vertical, view).astype(float) # 法向量

    if np.all(xdir == 0): # all(true)
        raise ValueError("vertical is parallel to v") # 平行法向量为0
